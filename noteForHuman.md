...
* 설치: `bash install.sh` — rc(zshrc·bashrc)에 fpm 부트스트랩 블록 + `~/.info/__pmBasePath.txt` 생성 (멱등)
* 제거: `bash uninstall.sh` — rc 블록 + `~/.info/__pmBasePath.txt` 를 백업 후 제거 (멱등, 흔적 없으면 no-op)
    - 백업 위치: `<repo>/_doc_work/z_done/fpm-uninstall-<YYYYMMDD_HHMMSS>/` (env `FPM_BACKUP_DIR` 로 변경 가능)
    - **보존**: `projects/`·`Projects.md`·`Servers.md` (사용자 데이터 — 자동 삭제 안 함)
* 클린 재설치: `bash install.sh --clean` — uninstall(백업+제거) 후 재설치를 한 번에
* 정리 범위 = 셸 아티팩트만 (Claude Code 플러그인 SCAR 는 `/plugin` 이 별도 관리)
* 소스=___pm 루트 3파일(install.sh·uninstall.sh·INSTALL.md) → fpm-sync 로 미러 전파. `_doc_work/` 백업물은 미러 제외(비공개)

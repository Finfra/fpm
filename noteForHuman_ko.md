...
## board (c모드, dashboard)
SSOT: `_doc_arch/hub_board_tmux_design.md` / Ops: `~/.claude/agents/dashboard.md`, `.claude/commands/dashboard.md`
...
| 🟠 **restart** | hub 서버가 **기동할 때 한 번만** 읽음 | **서버 재시작 필요**. `/board-server restart` 를 해야 반영 (현재 `bind_host` 만 해당) |

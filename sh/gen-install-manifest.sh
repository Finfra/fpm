...
#
# scar-manifest.yml(SSOT) 의 shell + payloads.plugin 값을 bash sourceable 한
...
#       bash sh/gen-install-manifest.sh --check    생성 결과와 현 .sh 가 일치하는지만 검사(쓰기 X)
...
if [[ "$MODE" == "check" ]]; then
    if [[ -f "$OUT" ]] && diff -q <(printf '%s\n' "$GENERATED") "$OUT" >/dev/null 2>&1; then
        info "✅ install_manifest.sh 는 yml 과 일치 (drift 없음)"
        exit 0
    else
        err "⚠️ drift 감지 — install_manifest.sh 가 yml 과 불일치. 'bash sh/gen-install-manifest.sh' 재생성 필요"

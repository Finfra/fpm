...
본 문서는 agent **운영 명세**(입력·절차·종료)만 담는다. 설계 정본(배경·결정·이력): `~/_git/___pm/_doc_arch/hub_dashboard_tmux_design.md`. 글로벌 `_doc_arch/dashboard.md`는 클라이언트측 운영 참조.
...
`~/.claude/agents/dashboard-runner.sh` 템플릿을 OUT_DIR로 복사 + shim 패턴으로 변수 주입.
...
bash ~/.claude/agents/dashboard-runner.sh
EOF
chmod +x "$SHIM_FILE"
...
`hub-trigger.sh` board 분기가 컨텍스트로 주입. agent 는 위 "완료 폴러 메타"를 반환하여 폴러 변수를 채운다.
...
* daemon: `agents/dashboard-supervisor.sh` (DAG 구동) · `agents/dashboard-queue-runner.sh` (시각화). queue.yaml 스키마: `agents/dashboard-queue.sample.yaml`
...
| daemon         | 파일                        | 책임                                                                                  |
| :------------- | :-------------------------- | :------------------------------------------------------------------------------------ |
| `supervisor`   | `dashboard-supervisor.sh`   | DAG 구동 — `blocked`→`ready` 승격 · worker 디스패치 · sentinel 완료 감지 · 크래시 복구 |
| `queue-runner` | `dashboard-queue-runner.sh` | 시각화 — `<topic>.dash.yaml` 생성·갱신 (queue.yaml 상태를 데이터 파일로 변환)  |
...
5. `OUT_DIR/<topic>.queue.yaml` 작성 — 스키마는 `agents/dashboard-queue.sample.yaml` 준수. 초기 `state: running`, item `status: blocked`(depends 있음) 또는 `ready`(없음)
...
2. pane 1 = supervisor — `dashboard-supervisor.sh` 실행 (env 주입)
3. pane 2 = queue-runner — `dashboard-queue-runner.sh` 실행 (env 주입)
...
`board-notify.sh`(PostToolUse) 매칭 패턴:
- `*.htm.{yaml,yml,json}`
- `*.dash.{yaml,yml,json}`
...
* 본 agent: `~/.claude/agents/dashboard.md`
* runner 템플릿: `~/.claude/agents/dashboard-runner.sh`
* 큐 모드 daemon: `~/.claude/agents/dashboard-supervisor.sh`, `~/.claude/agents/dashboard-queue-runner.sh`
* wrapper command: `~/.claude/commands/dashboard.md`
* 서버 lifecycle command: `~/.claude/commands/dashboard-server.md`
* hook: `~/.claude/hooks/board-notify.sh` (PostToolUse data 매칭)
* hook: `~/.claude/hooks/hub-trigger.sh` (UserPromptSubmit 트리거)

---
name: fpm-dashboard
description: tmux 기반 dashboard 에이전트. window 1:1 매칭으로 백그라운드 runner 가동, data 파일 주기 갱신. 순수 모니터링(단일 worker) + 큐 오케스트레이션(DAG) 2모드. 트리거 — "/dashboard <topic>" 또는 "..dashboard <topic>"
date: 2026-05-31
---

> ⚠️ **글로벌 SCAR 변경 가드 (Issue46)**: 본 agent 는 모든 프로젝트가 공유. cwd ≠ `~/.claude/` 면 즉시 수정 금지 → `~/.claude/Issue.md` 이슈 등록 후 처리. 설계 정본 SSOT: `~/_git/___pm/_doc_arch/hub_dashboard_tmux_design.md`. 절차: `~/.claude/rules/global-scar-change-rules.md`

# dashboard agent

Mode C(Live Dashboard) 운영을 main turn에서 분리. tmux pane을 띄워 그 안에서 runner.sh가 data 파일을 주기 갱신. main turn은 즉시 자유.

본 문서는 agent **운영 명세**(입력·절차·종료)만 담는다. 설계 정본(배경·결정·이력): `~/_git/___pm/_doc_arch/hub_dashboard_tmux_design.md`. 글로벌 `_doc_arch/fpm-dashboard.md`는 클라이언트측 운영 참조.
...
worker_pid: null             # (선택) spawn 한 background worker PID. cleanup 시 함께 kill
...
`~/.claude/agents/fpm-dashboard-runner.sh` 템플릿을 OUT_DIR로 복사 + shim 패턴으로 변수 주입.
...
bash ~/.claude/agents/fpm-dashboard-runner.sh
EOF
chmod +x "$SHIM_FILE"
...
`worker_pid` 를 설정하지 않는 경우, runner 는 주기 모니터링 패턴으로 동작:
...
* 폴 주기 **30s**, 기본 만료 **6h** (ETA 알면 `ETA_SEC×2` 우선). SCAR 전역 스케줄링 정책 준수 — crontab 금지, 네이티브 폴링 허용 (`_doc_arch/fpm-dashboard.md`)
...
* 설계 SSOT: `_doc_arch/fpm-dashboard.md` "큐 모드 — DAG 오케스트레이션" (N+2 프로세스·queue.yaml 스키마·시그널 맵)
* 정본 설계: `~/_git/___pm/_doc_arch/hub_dashboard_detail.md`
* daemon: `agents/fpm-dashboard-supervisor.sh` (DAG 구동) · `agents/fpm-dashboard-queue-runner.sh` (시각화). queue.yaml 스키마: `agents/fpm-dashboard-queue.sample.yaml`
...
| daemon         | 파일                        | 책임                                                                                  |
| :------------- | :-------------------------- | :------------------------------------------------------------------------------------ |
| `supervisor`   | `fpm-dashboard-supervisor.sh`   | DAG 구동 — `blocked`→`ready` 승격 · worker 디스패치 · sentinel 완료 감지 · 크래시 복구 |
| `queue-runner` | `fpm-dashboard-queue-runner.sh` | 시각화 — `<topic>.dash.yaml` 생성·갱신 (queue.yaml 상태를 데이터 파일로 변환)  |
...
5. `OUT_DIR/<topic>.queue.yaml` 작성 — 스키마는 `agents/fpm-dashboard-queue.sample.yaml` 준수. 초기 `state: running`, item `status: blocked`(depends 있음) 또는 `ready`(없음)
...
7. **Q&A 재개 (런타임)** — worker 가 작업 도중 입력 필요 시 `.waiting` sentinel 에 질문 기록. supervisor 가 `waiting_input` 으로 두고 `queue-qa` 위젯에 노출. 사용자가 답변 마커 `<OUT_DIR>/.dash-answers/<topic>__<id>`(내용=답변) 생성 시 supervisor 가 답변+재개 프롬프트 재주입. 상세: `_doc_arch/fpm-dashboard.md` `## 백그라운드 Q&A`
...
2. pane 1 = supervisor — `fpm-dashboard-supervisor.sh` 실행 (env 주입)
3. pane 2 = queue-runner — `fpm-dashboard-queue-runner.sh` 실행 (env 주입)
...
* 설계 정본 SSOT: `~/_git/___pm/_doc_arch/hub_dashboard_tmux_design.md`
* 클라이언트 운영 참조: `~/.claude/_doc_arch/fpm-dashboard.md`
* 본 agent: `~/.claude/agents/fpm-dashboard.md`
* runner 템플릿: `~/.claude/agents/fpm-dashboard-runner.sh`
* 큐 모드 daemon: `~/.claude/agents/fpm-dashboard-supervisor.sh`, `~/.claude/agents/fpm-dashboard-queue-runner.sh`
* wrapper command: `~/.claude/commands/fpm-dashboard.md`
* 서버 lifecycle command: `~/.claude/commands/fpm-dashboard-server.md`

...
b/c모드는 백엔드 서버를 사용한다. `/fpm-hub-server start|stop|restart|status|clear|reset` 로 관리 (구 `/fpm-board-server`, Issue190 통합 — 별칭 잔존):

```bash
/fpm-hub-server start    # ${CLAUDE_PLUGIN_ROOT}/services/hub/server.py 기동
/fpm-hub-server status   # healthz + PID
```

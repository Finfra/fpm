...
| 분류 | 항목 |
| :--- | :--- |
| Commands | `fpm-hub`, `fpm-board`, `fpm-board-server`, `fpm-pm-new`, `fpm-pm-del`, `fpm-pm-update`, `fpm-pm-query`, `fpm-pm-do`, `fpm-cdf` |
...
b/c모드는 백엔드 서버를 사용한다. `/fpm-board-server start|stop|status|restart` 로 관리:

```bash
/fpm-board-server start    # ${CLAUDE_PLUGIN_ROOT}/services/hub/server.py 기동
/fpm-board-server status   # healthz + PID
```

...
> ⚠️ **글로벌 SCAR 변경 가드 (Issue46)**: 본 커맨드는 모든 프로젝트가 공유. cwd ≠ `~/.claude/` 면 즉시 수정 금지 → `~/.claude/Issue.md` 이슈 등록 후 처리. 영속 설계 SSOT: `~/.claude/_doc_arch/board.md`. 절차: `~/.claude/rules/global-scar-change-rules.md`
...
* agent: `~/.claude/agents/fpm-board.md`
* runner (순수 모니터링): `~/.claude/agents/fpm-board-runner.sh`
* supervisor·runner (큐 모드, Issue84): `~/.claude/agents/fpm-board-supervisor.sh`, `~/.claude/agents/fpm-board-queue-runner.sh`
* queue.yaml 스키마: `~/.claude/agents/fpm-board-queue.sample.yaml`

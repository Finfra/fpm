...
| `..dashboard <topic>` / `..hub dash <topic>` | 순수 모니터링 — `hub-trigger.sh` 자동 감지 → agent 호출 |
| `/dashboard --queue <items>@<conc>`  | 큐 오케스트레이션 — 본 wrapper                                 |
| `..dashboard queue <items>@<conc>`   | 큐 오케스트레이션 — `hub-trigger.sh` 자동 감지 → agent 호출    |
...
* agent: `~/.claude/agents/dashboard.md`
* runner (순수 모니터링): `~/.claude/agents/dashboard-runner.sh`
* supervisor·runner (큐 모드, Issue84): `~/.claude/agents/dashboard-supervisor.sh`, `~/.claude/agents/dashboard-queue-runner.sh`
* queue.yaml 스키마: `~/.claude/agents/dashboard-queue.sample.yaml`

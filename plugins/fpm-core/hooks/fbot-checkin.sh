...
# ⚠️ env 명시 전달만 받는다. SessionStart 시점에는 프롬프트가 아직 훅에 오지 않으므로
#   "위임문에서 요지 추출"(Issue441 후보ⓑ)은 이 자리에서 원리적으로 불가능하다.
#   값이 없으면 NULL 을 유지한다 — 추측으로 채우면 낡은 값보다 나쁜 거짓이 된다.
[ -n "${FBOT_TASK:-}" ] && fbot_state set-task --bot-id "$FBOT_ID" --task "$FBOT_TASK"

...
# HR 게이트 경유 — 신규 기동은 스폰 판정 단일 SSOT 를 탄다 (Issue436_3 T4, 2026-08-24)
...
* 테스트 override: `FBOT_HR_GATE` env 로 게이트 경로 대체 가능(모의 거부·부재 시뮬레이션용)

# 위임 윈도우 GC (Issue401, 2026-08-16)

위임이 만든 `pm:` 윈도우는 **작업이 끝나도 남는다** — 워커 Claude 는 `-p` 1-shot 이라 죽지만 껍데기 zsh 가 살아 있다. 회수 코드가 없어 위임할수록 쌓였다.

## 회수 조건 — 3중, 하나라도 어긋나면 건드리지 않는다

| # | 조건 | 왜 |
| :-- | :--- | :--- |
| ① | pane 에 **claude 자식 0** | 살아 있으면 작업 중이다. 절대 kill 금지 |
| ② | 마지막 활동 후 **유예 경과**(기본 120분) | `--status`(capture-pane)로 스크롤백을 볼 실익을 죽이지 않는다 |
| ③ | **비활성** 윈도우 | 사용자가 보고 있을 수 있다 |

* 대상은 **`pm` 세션 한정**이고 **`kill-session` 은 쓰지 않는다**(윈도우만). 다른 tmux 세션 불가침
* 실행 시점은 **위임 직전** — `pm` 을 실제로 쓰는 순간이라 hook 배선이 필요 없다(예산 무관, [hook-rules](../../_doc_arch/rules-ondemand/hook-rules-detail.md) 규칙3 준수). 실패해도 위임은 계속된다(`|| true`)
* 수동: `fpm-do --gc` · 임계 조정: `PM_DO_GC_MINUTES=0 fpm-do --gc`(즉시 전부)

## 왜 무한 증가는 아니었나 (그래도 GC 가 필요한 이유)

윈도우 이름이 prj 별 고정(`pmdo{N}`·`cdft` 의 `WIN_NAME`)이라 같은 prj 재위임 시 **재사용**된다 → 상한은 **등록 prj 수 ≤ 45**. 무한은 아니지만 껍데기가 **영구히** 남고, 45개면 윈도우 목록이 실사용에 방해된다.
...
1. 위임 후 워커 세션이 뜰 때까지 둔다 (기동에 수 초 — 실측 9초 시점에 목록 등장)
2. `ListAgents` 로 워커를 특정한다. **이름만으로 고르지 말 것** — 같은 prj 세션이 여럿일 수 있다. 행에 함께 나오는 **tmux 좌표와 경과 시간**으로 가른다:
    ```
    common-1a [feb1da] · interactive · tmux pm:@2.%2 · started 9s ago   ← fpm-do 가 방금 만든 pm: 윈도우
    common-f1 [76dd3d] · interactive · started 6h ago                    ← 무관한 기존 세션
    ```
    fpm-do 는 자기가 만든 `pm:<win>` 을 알고 있으므로 **tmux 좌표 + 방금 뜸** 조합이면 오지목이 없다 🟢 (2026-08-16 실측, 2회 모두 성공)
    - ⚠️ **`tmux` 필드는 조건부다** — **tmux 안에서 도는 세션에만** 붙는다. VSCode·터미널 직접 실행 세션에는 없다(prj1 워커가 "필드에 tmux 좌표가 없다"고 보고했는데, 그 시점 목록에 tmux 세션이 하나도 없었기 때문이다 — 양쪽 관측이 모두 사실이었다)
    - 그래서 **이 방법은 `fpm-do` 위임에 한해 성립**한다. fpm-do 가 워커를 tmux 에 띄우기 때문이다. 일반 세션을 지목할 때는 쓸 수 없다

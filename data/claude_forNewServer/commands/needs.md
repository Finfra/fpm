...
> **규칙 로드**: 실행 전 `~/_git/___pm/_doc_arch/sp-nptir-rules.md` 를 Read할 것.
...
* 규칙 로드: [`~/_git/___pm/_doc_arch/sp-nptir-rules.md`](~/_git/___pm/_doc_arch/sp-nptir-rules.md) 신호 판정 기준표
* **`_doc_arch` 자동 지정** (Issue145 — 추가 프롬프트 없이 needs→설계문서 연결): Glob 으로 `_doc_arch/{주제}.md` 탐색.
    - **존재** → 4단계 frontmatter 에 `arch: _doc_arch/{주제}.md` 자동 주입 (확인 생략).
    - **없음** + 탐색 중 설계 결정 발생(triage `중간`·`복잡`) → 스켈레톤 자동 생성 제안 (Y/N 1회). Y → 작성 + `arch:` 주입 / N → 생략.
    - **없음** + 단순 구현(triage `단순`) → `arch:` 생략.
    - 상세 절차·스켈레톤 템플릿: [`~/.claude/_doc_arch/rules-ondemand/nptir-rules.md`](../_doc_arch/rules-ondemand/nptir-rules.md) "needs → `_doc_arch` 자동 지정 규칙".
...
date: {오늘 YYYY-MM-DD}
issue: TBD
arch: _doc_arch/{주제}.md   # 1단계 자동 지정에서 _doc_arch 연결 시만 주입, 없으면 생략
---
```
...
* [`~/_git/___pm/_doc_arch/sp-nptir-rules.md`](~/_git/___pm/_doc_arch/sp-nptir-rules.md) — superpowers × nPTiR 통합 규칙

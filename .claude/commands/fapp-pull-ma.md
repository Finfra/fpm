---
title: "FAPP-PULL-MA: ma에서 fApp 프로젝트 일괄 pull"
description: "ma에서 fApp 프로젝트 일괄 pull 커맨드"
date: 2026-03-28
---
...
for num in "${NUMS[@]}"; do
  ...
  PULL_LIST="${PULL_LIST}\n- ${project_name} (경로: ${path})"
done
echo -e "pull 대상:${PULL_LIST}"
```

## Step 2: sma 에이전트로 ma에서 각 프로젝트 pull

`sma` 에이전트에 위임하여 ma에서 각 fApp 프로젝트를 pull.

sma 에이전트에 전달할 내용:
* pull 대상 프로젝트 목록 (Step 1에서 확인한 목록)
* 각 프로젝트에 대해: `cd ${path} && git stash && git pull && git stash pop 2>/dev/null || true`
* 프로젝트별 pull 결과 보고

## Step 3: 완료 보고

```bash
say "fApp pull on ma complete"
```

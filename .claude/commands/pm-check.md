---
name: pm-check
description: 등록 프로젝트의 nPTiR 필수 폴더(특히 _doc_work/htm) 무결성 검사·자동 생성. hub 403 dead link 예방책.
date: 2026-07-09
---
...
특히 **`_doc_work/htm/`** 부재는 hub 렌더 산출물이 `/tmp/___pm` 로 fallback → register 훅
미매칭 → `/htm-doc` 403 dead link 를 유발하므로, 본 커맨드가 사전 방지한다.

> 근본 배경: hub 트리거(`~/.claude/hooks/fpm-hub-trigger.sh`)는 `$cwd/_doc_work/` 하위 htm 폴더가
> 있을 때만 프로젝트 로컬에 저장하고, register 훅(`fpm-hub-doc-register`)은 그 경로만 등록한다.
> 두 훅의 계약이 htm 폴더 존재를 전제하므로, 모든 등록 프로젝트에 `htm/` 을 보장하는 것이 예방책이다.
> Issue289 이후 활성 폴더는 `htm/`, 아카이브는 `z_done/htm/`, `z_htm/` 은 읽기 전용 legacy.
...
| prj | 경로 | 결과 |
| :-- | :--- | :--- |
| 5   | ~/_git/___common | 생성: _doc_work/htm |
| 7   | ~/_git/__all/fpm | OK |
```
...
* scaffold SSOT: `.claude/skills/pm/SKILL.md` 멱등 처리 매트릭스 (신규·adopt 시 htm 생성)
* htm 수명주기 SSOT: `_doc_arch/htm-lifecycle-design.md` (활성 `htm/` → 아카이브 `z_done/htm/`)

...
| mermaid 노드 **hover** | 팝업(프로젝트명 + `🗺️ 이슈 맵` 버튼) — 아래 참조 |

# 노드 hover 팝업 (Issue371)

맵은 **프로젝트 축**의 선수 관계를 보여준다. 거기서 **이슈 축**(그 프로젝트 안의 이슈 의존 관계도)으로 내려가는 통로를 노드에 붙였다 — hub 로 되돌아가 카드를 찾을 이유가 없어진다.

* 커서를 팝업으로 옮겨도 유지된다(유예 260ms). 그래야 버튼을 누를 수 있다 — 노드 자식 간 이동(라벨↔테두리)은 이탈로 치지 않는다(`relatedTarget` 판정)

## 3단 신호 (Issue372)

**"관계도가 있는가"와 "열 문서가 있는가"는 다른 질문**이다. 그래프가 비어도 그 문서에는 이슈 목록·완료 이력이 그대로 있고 서버도 정상 serve 한다(serve 판정은 `_issue_map_path` — 그래프를 보지 않는다). 그래서 둘을 갈라 3단으로 표현한다.

| 상태 | 판정 필드 | 팝업 버튼 | 노드 테두리(hover 불필요) |
| :--- | :--- | :--- | :--- |
| ① 맵 문서 없음 | — | 가장 흐림(0.35) · **링크 없음** | 그대로 |
| ② 문서만 있음(간선 0) | `issue_map_file` | 덜 흐림(0.62) · **링크 있음** | **왼쪽** 점선 |
| ③ 문서 + 선수 관계 그래프 | `issue_map` | 밝게 · 링크 | **왼쪽 + 위쪽** 점선 |

* 판정은 서버 `_issue_map_exists`/`_issue_map_visible` **단일 지점**이라 hub 카드 🗺️ · Project List `Map` 컬럼과 어긋나지 않는다
* 노드 테두리는 `<line>` 오버레이다 — SVG `rect` 는 **한 변만** 파선으로 만들 수 없다(`stroke-dasharray` 는 둘레 전체 적용). 세션 배지가 `<text>` 를 얹는 것과 같은 방식이며 `pointer-events:none` 이라 노드 클릭을 가리지 않는다
* 데이터는 **런타임 fetch**(60초 주기) — 이슈맵은 `Projects.md` 보다 자주 생기고 낡으므로 빌드 시점에 박으면 여는 순간 거짓이 된다. 활성 세션 오버레이와 같은 판단
* 스크롤·리사이즈 시 팝업은 노드를 **따라간다**(닫지 않는다 — 닫으면 스크롤 직후 막 연 팝업까지 사라진다)
* **hub serve 전제** — `file://` 직접 열기에서는 `/projects-list` fetch 가 실패해 팝업이 뜨지 않는다(맵 본체도 마찬가지)
